...
def update_rate(sender, instance, created, **kwargs):
    if created:
        ...
        average = Rate.objects.filter(car_id=car_id).aggregate(Avg('rate'))['rate__avg']
        rate_count = Rate.objects.filter(car_id=car_id).count()
        Car.objects.filter(pk=car_id).update(avg_rate=round(average, 4), rate_count=rate_count)
